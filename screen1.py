...
# api_key = "c350f6f5a4396d349ee4bbacde3d5999"
    ...
    for ticker in tickers:
        ...
        try:
            ...
            # Got all the data so go ahead and caluclate earnings power
            ...
            try:
                #  base case forecast
                forecast, inputs = get_ep(ticker = ticker, inc = inc, bs = bs, cf = cf)

                # covid 19 "down" scenario
                revenue_scenario_down = [-.2, -.07, .08, .035, .035]
                forecast_down, inputs_down = get_ep(ticker = ticker, inc = inc, bs = bs, cf = cf, revenue_scenario = revenue_scenario_down)

                # sector based covid19 scenario from above
                try:
                    revenue_scenario_sector = rscens[ttfdf.short_sector[ticker]]
                except:
                    revenue_scenario_sector = rscens[profile.sector[0]]
                else:
                    revennue_scenario_sector = []
                forecast_sector, inputs_sector = get_ep(ticker = ticker, inc = inc, bs = bs, cf = cf, revenue_scenario = revenue_scenario_sector)
            except:
                logging.info('forecast failed: {}'.format(ticker))
            ...
            tests = {
                    ...
                    'gm_ltm_test' : .555943, # June 2020 top quartile, median = .345134
                    'dividend_growth_3_test': .05, # June 2020 Top quartile is 0.011755
                    'ebitda_margin_ltm_test': .184, # June 2020 Top quartile
                    'sgam_ltm_test': .1945, # June 2020 median, top quartile = .085315
                    'capex_to_revenue_avg_3_test': .009, # June 2020 top quartile, median = .028490
                    'roic_change_from_ic_test' : 0,
                    'revenue_growth_3_test': .124, # June 2020 median. Top quartile = .4126
                    ...
                    'surplus_cash_as_percent_of_price_test':.123, # June 2020 top quartile
                    'icf_avg_3_to_fcf_test': 1,
                    'icf_to_ocf_test': .142, # June 2020 top quartile
                    'equity_sold_test': 2.55, # June 2020 top quartile
                    'financing_acquired_test': 42, # June 2020 median
                    'short_interest_ratio_test': 7.26, # June 2020 top quartile
                    'insider_ownership_total_test': .182050, # June 2020 top quartile
            ...
            # valuation tests
            df['ev_to_ebitda_ltm_test'] = df.ev_to_ebitda_ltm <= tests['ev_to_ebitda_ltm_test']
            df['pe_to_mid_cycle_ratio_test']  = df.pe / df.pe_mid_cycle <= tests['pe_to_mid_cycle_ratio_test']
            df['price_change_52_test'] = df.price_change_52 <= tests['price_change_52_test']
            valuation_tests = ['pe_to_mid_cycle_ratio_test', 'ev_to_ebitda_ltm_test', 'price_change_52_test']
            df['VALUATION_test'] = df[valuation_tests].mean().sum() / len(valuation_tests)

            # superior business model tests
            df['roic_high_5_test'] = df.roic_high_5 >= tests['roic_high_5_test']
            df['dividend_growth_3_test'] = df.dividend_growth_3 >= tests['dividend_growth_3_test']
            df['gm_ltm_test'] = df.gm_ltm >= tests['gm_ltm_test']
            df['em_margin_ltm_test'] = df.ebitda_margin_ltm >= tests['ebitda_margin_ltm_test']
            df['sgam_ltm_test'] = df.sgam_ltm <= tests['sgam_ltm_test']
            df['revenue_growth_3_test'] = df.revenue_growth_3 > tests['revenue_growth_3_test']
            df['revenue_growth_max_test'] = df.revenue_growth_3 > tests['revenue_growth_max_test']
            try:
                df['market_leader_test'] = ttfdf.market_leader_test[ticker] >= tests['market_leader_test']
            except:
                df['market_leader_test'] = False
            ...
            df['SBM_test'] = df[sbm_tests].mean().sum() / len(sbm_tests)

            # poor use of cash tests / watch-outs (lower is better)
            df['capex_to_revenue_avg_3_test'] = df.capex_to_revenue_avg_3 >= tests['capex_to_revenue_avg_3_test']
            df['roic_change_from_ic_test'] = df.roic_change_from_ic <= tests['roic_change_from_ic_test']
            df['surplus_cash_as_percent_of_price_test'] = df.surplus_cash_as_percent_of_price >= tests['surplus_cash_as_percent_of_price_test']
            df['additional_debt_from_ebitda_multiple_per_share_test'] = df.additional_debt_from_ebitda_multiple_per_share >= tests['additional_debt_from_ebitda_multiple_per_share_test']
            df['icf_avg_3_to_fcf_test'] = df.icf_avg_3_to_fcf >= tests['icf_avg_3_to_fcf_test']
            df['icf_to_ocf_test'] = df.icf_ltm / df.ocf_ltm >= tests['icf_to_ocf_test']
            df['equity_sold_test'] = df.equity_sold >= tests['equity_sold_test']
            df['financing_acquired_test'] = df.financing_acquired >= tests['financing_acquired_test']
            df['capex_to_ocf_test'] = df.capex_to_ocf >= tests['capex_to_ocf_test']
            puoc_tests = [
            ...
            df['PUOC_test'] = df[puoc_tests].mean().sum() / len(puoc_tests)

            # balance sheet risk tests (lower is better)
            df['icf_and_ocf_negative_test'] = (df.icf_ltm < 0) & (df.ocf_ltm < 0)
            df['net_debt_to_ebitda_ltm_test'] = df.net_debt_to_ebitda_ltm >= tests['net_debt_to_ebitda_ltm_test']
            df['ebitda_to_interest_coverage_test'] = df.ebitda_to_interest_coverage <= tests['ebitda_to_interest_coverage_test']
            bs_risks_tests = [
                'icf_and_ocf_negative_test',
                'net_debt_to_ebitda_ltm_test',
                'ebitda_to_interest_coverage_test',
                ]
            df['BS_risks_test'] = df[bs_risks_tests].mean().sum() / len(bs_risks_tests)

            # trading issues tests (lower is better)
            df['short_interest_ratio_test'] = df.short_interest_ratio >= tests['short_interest_ratio_test']
            df['insider_ownership_total_test'] = df.insider_ownership_total >= tests['insider_ownership_total_test']
            df['insiders_can_get_out_quickly_test'] = df.insider_ownership_total / df.adv_as_percent_so >= tests['insiders_can_get_out_quickly_test']
            df['adv_avg_months_3_test'] = df.adv_avg_months_3 <= tests['adv_avg_months_3_test']
            df['float_test'] = df.float <= tests['float_test']
            df['insiders_selling_ltm_test'] = df.insiders_selling_ltm <= tests['insiders_selling_ltm_test']
            trade_tests = [
            ...
            df['TRADE_test'] = df[trade_tests].mean().sum() / len(trade_tests)

            # performance improvement opp tests
            df['price_opp_at_em_high_test'] = df.price_opp_at_em_high >= tests['price_opp_at_em_high_test']
            df['price_opp_at_sgam_low_test'] = df.price_opp_at_sgam_low >= tests['price_opp_at_sgam_low_test']
            df['price_opp_at_roic_high_test'] = df.price_opp_at_roic_high >= tests['price_opp_at_roic_high_test']
            pi_tests = [
                'price_opp_at_roic_high_test',
                'price_opp_at_sgam_low_test',
                'price_opp_at_em_high_test',
                ]
            df['PI_test'] = df[pi_tests].mean().sum() / len(pi_tests)
            ...
            # earnings power elements
            try:
                df['sell_price'] = forecast.T.value_per_share[5]
                df['ep_irr'] = ((df.sell_price / df.price) ** ( 1 / 5)) -1
                df['buy_price_ten_percent'] = df.sell_price / ((1 + inputs.ep_discount) ** 5)
                df['implied_ebitda_multiple'] = forecast.T.ev[5] / forecast.T.ebitda[5]
                df['ep_today'] = forecast.T.value_per_share[1]
                # Put in down scenarios
                df['sell_price_down'] = forecast_down.T.value_per_share[5]
                df['ep_irr_down'] = ((df.sell_price_down / df.price) ** ( 1 / 5)) -1
                df['buy_price_ten_percent_down'] = df.sell_price_down / ((1 + inputs_down.ep_discount) ** 5)
                df['implied_ebitda_multiple_down'] = forecast_down.T.ev[5] / forecast_down.T.ebitda[5]
                df['ep_today_down'] = forecast_down.T.value_per_share[1]
                # Put in down sector scenarios
                df['sell_price_sector'] = forecast_sector.T.value_per_share[5]
                df['ep_irr_sector'] = ((df.sell_price_sector / df.price) ** ( 1 / 5)) -1
                df['buy_price_ten_percent_sector'] = df.sell_price_sector / ((1 + inputs_sector.ep_discount) ** 5)
                df['implied_ebitda_multiple_sector'] = forecast_sector.T.ev[5] / forecast_sector.T.ebitda[5]
                df['ep_today_sector'] = forecast_sector.T.value_per_share[1]
            except:
                logging.warning('get ep failed: {}'.format(ticker))
...
def add_ep(d = pd.DataFrame(), ebitda_scenario = []):
    ...
    # assumes d is a single ticker record

    down = [-.2, -.07, .08, .035, .035]
    # Get the ticker name which should be the only item in the index (e.g. item 0)
    for i in d.index:
        print('working on ', i)
        try:
