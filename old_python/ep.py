...
bs['wc'] = (bs.totalCurrentAssets - bs.cashAndShortTermInvestments) - \
...
sga = (inc.revenue - inc.ebitda -
       inc.costOfRevenue).fillna(0).values.reshape(-1, 1)
...
inputs['da_to_revenue_5_median'] = [
    (cf.depreciationAndAmortization / inc.revenue).last('5Y').median()]
inputs['sbc_to_revenue_5_median'] = [
    (cf.stockBasedCompensation / inc.revenue).last('5Y').median()]
inputs['interest_rate'] = [
    (inc.interestExpense / bs.totalDebt).last('5Y').median()]
inputs['wc_to_revenue_5_median'] = [(bs.wc / inc.revenue).last('5Y').median()]
inputs['capex_to_revenue_5_median'] = [
    (cf.capitalExpenditure / inc.revenue).last('5Y').median()]
inputs['maintenance_capex_to_revenue'] = [
    (cf.capitalExpenditure / inc.revenue).last('5Y').min()]
inputs['revenue_growth_3_median'] = [
    inc.revenue.pct_change(1).last('3Y').median()]
inputs['ebitda_margin_3_median'] = [
    (inc.ebitda / inc.revenue).last('3Y').median()]
inputs['interest_placeholder'] = [
    (inc.interestExpense / inc.revenue).last('5Y').median()]
inputs['so'] = [inc.weightedAverageShsOutDil['2019'][0]]
# use last tax_rate given changes in 2019
inputs['tax_rate'] = [
    (inc.incomeTaxExpense / inc.incomeBeforeTax).last('1Y').median()]
inputs['dividend_payout_to_ebitda_ratio_3_median'] = [
    (cf.dividendsPaid / inc.ebitda).last('3Y').median()]
...
forecast.loc['sga', 0] = inc.revenue['2019'][0] - inc.ebitda['2019'][0] - \
    (inc.revenue['2019'][0] - inc.grossProfit['2019'][0])
...
for year in forecast.columns[1:6]:
    forecast.loc['revenue', year] = forecast.loc['revenue',
                                                 year - 1] * (1 + revenue_scenario[year - 1])

for year in forecast.columns[6:]:
    forecast.loc['revenue', year] = forecast.loc['revenue',
                                                 year - 1] * (1 + inputs['out_years_revenue_growth'][0])
...
if len(ebitda_scenario) > 1:
    for year in forecast.columns[1:]:
        forecast.loc['ebitda', year] = forecast.loc['ebitda',
                                                    year] * (ebitda_scenario[year - 1])
elif len(ebitda_scenario) == 1:
    for year in forecast.columns[1:]:
        forecast.loc['ebitda', year] = forecast.loc['ebitda',
                                                    year] * (ebitda_scenario[0])

# do non-cross-year calcs first
forecast.loc['da'] = forecast.loc['revenue'] * \
    inputs['da_to_revenue_5_median'][0]
forecast.loc['ebit'] = forecast.loc['ebitda'] - forecast.loc['da']
forecast.loc['maintenance_capex'] = forecast.loc['revenue'] * \
    inputs['maintenance_capex_to_revenue'][0]
forecast.loc['capex'] = forecast.loc['revenue'] * \
    inputs['capex_to_revenue_5_median'][0]
forecast.loc['wc'] = forecast.loc['revenue'] * \
    inputs['wc_to_revenue_5_median'][0]
forecast.loc['dividends'] = forecast.loc['ebitda'] * \
    inputs['dividend_payout_to_ebitda_ratio_3_median'][0]
forecast.loc['earnings_power'] = (
    forecast.loc['ebitda'] - forecast.loc['maintenance_capex']) * (1 - inputs['tax_rate'][0])

# do some cross-year calculations
for year in forecast.columns[1:]:
    forecast.loc['change_in_working_cap', year] = forecast.loc['wc',
                                                               year] - forecast.loc['wc', year - 1]
    forecast.loc['total_debt', year] = forecast.loc['total_debt', year -
                                                    1] - forecast.loc['ebitda', year] - forecast.loc['capex', year]
    forecast.loc['interest_expense', year] = max(
        0, forecast.loc['total_debt', year - 1] * inputs['interest_rate'][0])
    forecast.loc['cum_dividends', year] = forecast.loc['dividends',
                                                       year] + forecast.loc['dividends', year - 1]
...
for year in forecast.columns[1:]:
    forecast.loc['net_debt', year] = forecast.loc['net_debt',
                                                  year - 1] - forecast.loc['cash_available_to_pay_debt', year]
...
for year in forecast.columns[6:]:
    forecast.loc['incremental_ep', year] = forecast.loc['earnings_power',
                                                        year] - forecast.loc['earnings_power', 5]
    # put in extra capital required to maintain?
    forecast.loc['discounted_ep', year] = forecast.loc['earnings_power',
                                                       year] / ((1 + inputs['ep_discount'][0]) ** (year))
    forecast.loc['cumulative_ep', year] = forecast.loc['discounted_ep',
                                                       year] + forecast.loc['discounted_ep', year - 1]
    forecast.loc['tv'] = forecast.loc['cumulative_ep', year] / \
        inputs['ep_discount'][0]

forecast.loc['ev_in_year'] = forecast.loc['earnings_power'] / \
    inputs['ep_discount'][0]

for year in forecast.columns:
    forecast.loc['tv_value_in_year', year] = forecast.loc['tv',
                                                          10] / ((1 + inputs['ep_discount'][0]) ** (10 - year))

forecast.loc['ev'] = forecast.loc['ev_in_year'] + \
    forecast.loc['tv_value_in_year'] + forecast.loc['cum_dividends']
forecast.loc['implied_ebitda_multiple'] = forecast.loc['ev'] / \
    forecast.loc['ebitda']
forecast.loc['equity_value'] = forecast.loc['ev'] - forecast.loc['net_debt']
forecast.loc['value_per_share'] = forecast.loc['equity_value'] / \
    inputs['so'][0]
