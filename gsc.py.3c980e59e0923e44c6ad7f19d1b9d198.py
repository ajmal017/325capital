...
compare_series_bar(ax = ax, data_labels = actual_data, value_list = [ ticker_test_values , benchmark_test_values ], title = title, percent = False)
...
# The true/false values
values = [df1.loc[ticker,i] for i in test_data]
title = 'Superior Business Model tests =  {:2.1f}%'.format(sum(values)/ len(values) * 100 )
# The ticker's actual values to measure true-false with
ticker_test_values = [df1.loc[ticker,i] for i in actual_data]
# The benchmark values for true-false
benchmark_test_values = [testdict['tests'][i] for i in test_data]
compare_series_bar(ax = ax, data_labels = actual_data, value_list = [ ticker_test_values , benchmark_test_values ], title = title, percent = True)
...
# The true/false values
values = [df1.loc[ticker,i] for i in test_data]
title = 'Balance Sheet Risks tests =  {:2.1f}%'.format(sum(values)/ len(values) * 100 )
# The ticker's actual values to measure true-false with
ticker_test_values = [df1.loc[ticker,i] for i in actual_data]
# The benchmark values for true-false
benchmark_test_values = [testdict['tests'][i] for i in test_data]
...
# The true/false values
values = [df1.loc[ticker,i] for i in test_data]
title = 'Blueprint Execution tests =  {:2.1f}%'.format(sum(values)/ len(values) * 100 ) + '\n' + 'Status: {}, Work: {}, Peers: {}'.format(df1.loc[ticker, 'tamale_status'], df1.loc[ticker, 'last_work'], df1.loc[ticker, 'sagard_peers'])
# The ticker's actual values to measure true-false with
ticker_test_values = [df1.loc[ticker,i] for i in actual_data]
# The benchmark values for true-false
benchmark_test_values = [testdict['tests'][i] for i in test_data]
